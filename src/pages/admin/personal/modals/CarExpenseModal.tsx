...
import type { GarageExpenseRow, GarageExpenseKind } from "../../../../lib/garage/types";
import { addCarExpense, updateCarExpense } from "../../../../lib/garage/api";
...
export function CarExpenseModal(props: {
  ...
  useEffect(() => {
    ...
    if (initial) {
      ...
      setKind((initial.kind || "general") as GarageExpenseKind);
  ...
  const canSave = useMemo(() => {
    ...
    return true;
  }, [busy, date, name, amount, currency]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-3xl bg-white shadow-xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-100 p-5">
          <div>
            <div className="text-lg font-semibold">{isEdit ? "Edit Expense" : "Add Expense"}</div>
            <div className="mt-1 text-sm text-slate-600">Name, vendor, kind, amount.</div>
          </div>
        ...
        <div className="p-5 space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <label className="text-sm">
              <div className="mb-1 text-slate-600">Vendor (optional)</div>
              <input
                value={vendor}
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (isEdit && initial?.id) {
                  await updateCarExpense(initial.id, {
                    date,
                    name: name.trim(),
